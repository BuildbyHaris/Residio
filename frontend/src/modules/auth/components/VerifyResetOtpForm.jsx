...
import { verifyResetOtp, resendOtp } from "../services/auth.service";
...
const VerifyResetOtpForm = () => {
  ...
  const resolveOtpExpires = () => {
    const stored = sessionStorage.getItem(OTP_EXPIRES_KEY);

    // Use the latest saved expiry if available
    if (stored) {
      return stored;
    }

    // Otherwise use the value passed from the previous page
    if (location.state?.otpExpires) {
      sessionStorage.setItem(
        OTP_EXPIRES_KEY,
        location.state.otpExpires
      );
