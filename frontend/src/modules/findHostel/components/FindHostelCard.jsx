...
import { createOrGetPropertyConversation } from "../../chat/services/chat.service";
...
const FindHostelCard = ({ hostel }) => {
  ...
  const isVerifiedFlag = isVerified || verified;

  const displayPrice = startingPrice || price || 0;

  const displayLocation =
    [location, city].filter(Boolean).join(", ") || address || "—";

  const cardAmenities = amenities.slice(0, 4);
  ...
  const handleChatWithOwner = async () => {
    try {
      ...
      const response = await createOrGetPropertyConversation(hostelId);
  ...
  return (
    <div className="bg-white border border-[#EDEDED] rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-col">
      ...
      <div className="relative">
        <img src={cover} alt={name} className="w-full h-[170px] object-cover" />
        ...
        >
          <Heart
            className={`w-4 h-4 ${
              fav ? "fill-[#F5732C] text-[#F5732C]" : "text-gray-500"
            }`}
          />
        </button>
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        {cardAmenities.length > 0 && (
          <div className="flex items-center gap-3 mt-3 text-gray-500">
            {cardAmenities.map((amenity) => {
              const Icon = AMENITY_ICON[amenity];

              if (!Icon) return null;

              return <Icon key={amenity} className="w-4 h-4" />;
            })}
          </div>
        )}
